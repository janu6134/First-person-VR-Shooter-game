...
public class GunVR : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ((OVRInput.GetDown(OVRInput.Button.PrimaryIndexTrigger)|| Input.GetMouseButtonDown(0) || OVRInput.Get(OVRInput.Button.PrimaryIndexTrigger)) && gunShotTime <= 0 && gunReloadTime <= 0.0f && magBulletsVal > 0 && !isDead)
        {
        ...
        if ((OVRInput.GetDown(OVRInput.Button.Back) || OVRInput.Get(OVRInput.Button.Back) || OVRInput.GetDown(OVRInput.RawButton.Back) || OVRInput.GetDown(OVRInput.RawButton.Back) || Input.GetKey(KeyCode.R) && gunReloadTime <= 0.0f && gunShotTime <= 0.1f && remainingBulletsVal > 0 && magBulletsVal < magSize && !isDead))
        {
    ...
    public void Loadcrate() //resetting the amount of mag bullets we have to the initial count 90
    {   
        remainingBulletsVal = 90;
        Debug.Log(remainingBulletsVal);
        updateText();
    }
}
